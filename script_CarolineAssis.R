...
ndiffs(federal.ts)
# também podemos aplicar uma transformação, nesse caso vamos usar a 
#transformação log
federal.ts_transf = ts(log(federal.ts), start = 1990, end = 2019, 
                       frequency = 1)
ndiffs(federal.ts_transf)
# vemos que mesmo com a transformação continua sendo não estácionária, então
#vamos partir para as diferenciações
